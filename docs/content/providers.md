...
Credential source precedence is `auth_token` > non-empty `auth_env` > `auth_command`. `auth_command` is an argv array executed directly, never through a shell; it must print exactly one credential line to stdout.
