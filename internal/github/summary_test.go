...
func TestRenderSummaryFooterVersion(t *testing.T) {
	...
	if !strings.Contains(withVer, "Posted by [miu-cr](https://github.com/vanducng/miu-cr) v0.40.0</sub>") {
		t.Fatalf("footer must append the version when set: %s", withVer)
	}
	empty := RenderSummaryFull(&PRInfo{HeadSHA: "deadbeef"}, nil, nil, 0, nil, nil, SummaryOptions{})
	if strings.Contains(empty, "miu-cr) v") {
		t.Fatalf("empty Version must not append a version token: %s", empty)
	}
}
