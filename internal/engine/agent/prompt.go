...
- WRITING STYLE for every prose field (title, rationale, walkthrough, confidence_reason): plain, direct, technical English a busy engineer can skim. Do NOT use em dashes or en dashes (the "—" and "–" characters); use a period, comma, colon, or parentheses. Avoid filler openers ("moreover", "furthermore", "it is worth noting", "note that", "additionally"), hedging, and marketing adjectives ("robust", "seamless", "powerful", "leverage", "delve", "comprehensive"). Lead with the concrete problem and name specifics (the symbol, value, or path); cut any sentence that does not add information. Wrap every code identifier (column, table, model, function, file path, config key, env var, CLI flag, literal value) in single backticks so it renders as monospace: write ` + "`delivered_at`" + `, not delivered_at. Structure the rationale into short, scannable lines, never one dense block. Lead with the problem in one short sentence, add the mechanism and impact in one or two more short sentences, and put any verification question on its own final line. Separate each of these parts with a BLANK line: on GitHub a blank line renders as a real break, but a single newline collapses to a space, so a rationale written as one run-on paragraph becomes an unreadable wall. When a finding has several distinct points, write them as a short markdown bullet list (each line starting with "- ") instead of packing them into one sentence. At most two sentences per line or bullet.

You MAY also optionally include, alongside the findings, a short PR-level "walkthrough" formatted as up to 5 short key-point bullets (each a line starting with "- ") describing at a HIGH LEVEL what the change does and why, one short line each (the headline change, not low-level implementation detail). Lead with the MOST SUBSTANTIVE change by behavioral impact, not a trivial config or formatting tweak that happens to be listed first. When a large diff is mostly mechanical (auto-formatting, renames, generated code), say so plainly and point the reader to the small substantive core, so a big line count does not read as a big change. Also emit a "file_summaries" object mapping each changed file path (verbatim from its File header) to a one-line note. Both are optional context only — keep them brief, never let them replace or alter the findings array, and omit them if you have nothing useful to add. Also optionally include a "confidence" integer 1–5 (your confidence the change is safe to merge: 5 = very safe, 1 = risky) and a one-line "confidence_reason" justifying it.

Respond with a single JSON object, no prose, no markdown fences:
{"findings":[{"file":"<path from the File header>","existing_code":"<verbatim quoted code>","severity":"high","category":"bug","title":"<optional short title>","rule":"<optional motivating rule stem>","rationale":"<why this is a problem, as blank-line-separated short lines: the problem, then mechanism/impact, then any confirm question>","suggested_patch":"<optional fix>"}],"walkthrough":"<optional short summary>","file_summaries":{"<path>":"<optional one-line note>"},"confidence":<optional 1-5>,"confidence_reason":"<optional one line>"}
...
func stripCodeFences(s string) string {
	t := trimBlankBoundaryLines(strings.NewReplacer("\r\n", "\n", "\r", "\n").Replace(s))
	if t == "" {
		return ""
	}
	fenced := strings.TrimLeft(t, " \t")
	if !strings.HasPrefix(fenced, "```") {
		return t
	}
...
func trimBlankBoundaryLines(s string) string {
