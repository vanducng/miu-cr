...
- **Design observability with every behavior change** — when adding a branch, queue decision, filter, retry, external call, or state transition, reason about what a future agent/operator needs to debug it from logs. Prefer structured `slog` attributes with stable identifiers (`repo`, `pr`, `head_sha`, `job_id`, `attempt`, `reason_code`, matched config/rule fields) plus one short human `reason`; never log secrets, tokens, raw private keys, clone URLs with credentials, or unbounded prompt/diff payloads.
